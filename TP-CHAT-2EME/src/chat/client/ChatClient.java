...
public class ChatClient implements Runnable
{
	                  ...
	                  Logger parentLogger)
	{
		...
		if (serverOut != null) {logger.info("ChatClient: got client output stream to server");}
		}
		catch (IOException e)
		{
		logger.severe("ChatClient: " + Failure.CLIENT_OUTPUT_STREAM);
		}
		if (serverOut == null) {logger.severe("ChatClient: null server out" + Failure.CLIENT_INPUT_STREAM);}

		/*Create a temporary PrintWriter to serverOut (serverOutPW with
		 * autoFlush) and
		...
		if (serverOut != null)
		{
			...
			if (serverOutPW.checkError()) {logger.severe("serverOutPW : Error in println");}
			
			logger.info("ChatClient: sending name to server ... ");

			//use serverOutPW to println user name and check for errors
		}

		/*
		 * get input steam from clientSocket
